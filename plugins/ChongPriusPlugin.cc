...
void ChongPriusPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  // ingition's transport node, subcribe /joy
  dPtr->node.Subscribe("/joy", &ChongPriusPlugin::OnPriusCommand, this);
  ...
  // gznode(gazebo's transport) subcribe keypress messages
  this->dataPtr->keyboardSub =
    this->dataPtr->gznode->Subscribe("~/keyboard/keypress",
        &ChongPriusPlugin::OnKeyPress, this, true);
  ...
  // node(ignition's transport) subcribe keypress messages
  this->dataPtr->node.Subscribe("/keypress", &ChongPriusPlugin::OnKeyPressIgn,
      this);
}

/////////////////////////////////////////////////
// for joystick operation, subcribe joy messages
void ChongPriusPlugin::OnPriusCommand(const ignition::msgs::Joy &_joyMsg)
{
  this->dataPtr->lastSteeringCmdTime = this->dataPtr->world->SimTime();
  this->dataPtr->lastPedalCmdTime = this->dataPtr->world->SimTime();

  if (_joyMsg.axes(THROTTLE_AXIS) >= 0)
  {
    // Throttle command
    this->dataPtr->gasPedalPercent =
      ignition::math::clamp(_joyMsg.axes(THROTTLE_AXIS), 0.0f, 1.0f);
  }
  else
  {
    // Brake command
    this->dataPtr->brakePedalPercent =
      ignition::math::clamp(_joyMsg.axes(THROTTLE_AXIS) * -1, 0.0f, 1.0f);
  }

  if (_joyMsg.buttons(3))
    this->dataPtr->directionState = ChongPriusPluginPrivate::FORWARD;
  else if (_joyMsg.buttons(1))
    this->dataPtr->directionState = ChongPriusPluginPrivate::NEUTRAL;
  else if (_joyMsg.buttons(0))
    this->dataPtr->directionState = ChongPriusPluginPrivate::REVERSE;

  float steer = _joyMsg.axes(STEERING_AXIS);

  // Steering wheel command
  double handCmd = steer < 0.0f ? steer * -this->dataPtr->handWheelLow
                                : steer * this->dataPtr->handWheelHigh;
...
/////////////////////////////////////////////////
void ChongPriusPlugin::KeyControlTypeA(const int _key)
{
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
...
/////////////////////////////////////////////////
void ChongPriusPlugin::KeyControlTypeB(const int _key)
{
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
  ...
  switch (_key)
  {
    ...
    {
      this->dataPtr->brakePedalPercent = 0.0;
      if (this->dataPtr->directionState != ChongPriusPluginPrivate::REVERSE)
        this->dataPtr->gasPedalPercent = 0.0;
      this->dataPtr->gasPedalPercent += 0.1;
      this->dataPtr->gasPedalPercent =
          std::min(this->dataPtr->gasPedalPercent, 1.0);
...
/////////////////////////////////////////////////
void ChongPriusPlugin::KeyControl(const int _key)
{
...
/////////////////////////////////////////////////
void ChongPriusPlugin::OnKeyPress(ConstAnyPtr &_msg)
{
  this->KeyControl(_msg->int_value());
}

/////////////////////////////////////////////////
void ChongPriusPlugin::OnKeyPressIgn(const ignition::msgs::Any &_msg)
{
  this->KeyControl(_msg.int_value());
}
