...
#ifndef GAZEBO_PLUGINS_PRIUSHYBRIDPLUGIN_HH_
#define GAZEBO_PLUGINS_PRIUSHYBRIDPLUGIN_HH_
...
{
  // Forward declaration
  class ChongPriusPluginPrivate;

  /// \brief A model plugin for prius hybrid
  class ChongPriusPlugin : public ModelPlugin
  {
    /// \brief Constructor.
    public: ChongPriusPlugin();

    /// \brief Destructor.
    public: virtual ~ChongPriusPlugin();
    ...
    /// \brief Prius command callback
    private: void OnPriusCommand(const ignition::msgs::Joy &_joyMsg);
    ...
    /// \brief Private data
    private: std::unique_ptr<ChongPriusPluginPrivate> dataPtr;
  };
}
#endif
